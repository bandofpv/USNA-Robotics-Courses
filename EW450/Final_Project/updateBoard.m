...
% Output 1: canGrab - Nx5 cell array of Piece AprilTags not on the board 
% (Tag Family, Tag ID, Tag Scale, 'O'/'X', 4x4 pose relative to camera)

function [board, canGrab] = updateBoard(imDist,cameraParams, board)
    ...
    % DELETE AFTER (PLOTS CENTER POSE)
    ...
    for i = 1:size(pieces, 1)
        ...
        if ~isempty(H_a2c)
            ...
            if onBoard(H_a2c, H_b2c)
                ...
                displayBoard(board); % DELETE LATER
            else
