...
class RRT:
    ...
    def convert_to_grid(self, point):
        ...
        # Image row 0 is map_y_max, row end is map_y_min
        x_idx = int((point[0] - self.x_limit[0]) / self.res)
        y_idx = int((self.y_limit[1] - point[1]) / self.res)
        return (x_idx, y_idx)
    ...
    def _is_collision_free(self, node):
        ...
        # Check if grid cell is occupied (0.0 means occupied in occupancy grid)
