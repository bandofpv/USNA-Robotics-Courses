...
% Output 1: H_b2o - 4x4 array specifying the pose of the Board frame (at
% board's center) relative to the UR3e base frame.

function [H_b2o] = getBoardPose(imDist,cameraParams, H_c2o)
    ...
    for i = 1:length(tags)
        ...
        tagID = tags{i, 2};
        tagSize = tags{i, 3};
        p_i2a = [tags{i, 4}; tags{i, 5}, 0]; % point at board's center relative to AprilTag
        
        % find AprilTag pose (use distorted image)
        H_a2c = getAprilTagPose(imDist,cameraParams,tagFamily,tagID,tagSize); 
    ...
    % % calculate AprilTag pose relative to UR3e base
    % H_a2o = H_c2o * H_a2c;

    % % Parse through detected board tags
    % for i = 1:length(tagPoses)
    %     % Check if pose exists
    %     if isempty(tagPoses{i})
    %         missingTag = True;
    %     else
    %         % translate tag poses to center of board
    %         tagPoses{i}(1:2, 4) = tagPoses{i}(1:2, 4) - [tags{i, 4} tags{i, 5}]';
    %         H_b2o = tagPoses{i};
    %     end
    % end
    ...
    if ~missingTag
        ...
        t_1 = tagPoses{1}(1:3, 4);
        t_2 = tagPoses{2}(1:3, 4);
